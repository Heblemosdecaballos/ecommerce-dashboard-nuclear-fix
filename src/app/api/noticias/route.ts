...
export async function GET() {
  ...
  const { data, error } = await supa!.from("news_posts").select("*").order("created_at", { ascending: false });
...
export async function POST(req: Request) {
  ...
  const { title, content, cover_path } = await req.json().catch(() => ({}));
  ...
  const { error } = await supa!.from("news_posts").insert({
    author_id: user?.id, title, content: content ?? null, cover_path: cover_path ?? null,
  });
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
